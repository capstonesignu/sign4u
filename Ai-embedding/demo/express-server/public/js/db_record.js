...
window._dbRecordingHook = function (results) {
  ...
  _dbStatusEl.textContent = `녹화 중... ${_dbBuffer.length} frames`;
};
...
function _showCaptureUI(videoURL) {
  ...
  _dbStatusEl.textContent = `${_dbCapture.length} frames 녹화됨 — 싱크 조정 후 타임라인으로 구간을 조절하세요`;
...
_dbSaveBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    const res = await fetch("/data/keypoints/save", {
      ...
      body: JSON.stringify({ keypoints: cropFrames.map(flattenKeypoints), fps: actualFps, label, recorder_name: recorderName }),
    });
    if (!res.ok) {
