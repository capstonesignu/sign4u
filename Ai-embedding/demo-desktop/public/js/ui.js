/**
 * ui.js — demo-desktop override
 * setFrameCount를 제외한 나머지는 원본과 동일.
 * setFrameCount(n, secs, fps) — 선택 인자로 시간·fps 추가 표시.
 */
...
function setFrameCount(n, secs, fps) {
  const el = document.getElementById("frame-counter");
  if (!el) return;
  if (secs !== undefined && fps !== undefined) {
    el.textContent = `${n}f  ${secs.toFixed(1)}s  ${fps.toFixed(1)}fps`;
  } else {
