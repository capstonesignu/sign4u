...
const NAV_ITEMS = [
  ...
  { icon: '/records.png',  label: '진료 기록', path: '/patient/records',  active: true  },
        ...
        {!loading && !error && filtered.map((record) => {
          ...
          return (
            <div key={record.id} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '16px', border: '1px solid #E5E7EB', borderRadius: '16px' }}>
              <div style={{ width: '52px', height: '52px', borderRadius: '50%', backgroundColor: '#DCFCE7', flexShrink: 0, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <img
                  src={record.partner_image || '/doctor.png'}
                  alt="doctor"
                  style={{ width: record.partner_image ? '100%' : '32px', height: record.partner_image ? '100%' : '32px', objectFit: 'cover' }}
                />
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '17px', fontWeight: 'bold', color: '#111827', marginBottom: '6px' }}>{record.partner_name} 의사</div>
