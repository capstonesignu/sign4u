          ...
          <>
            ...
            <div style={{ border: '1.5px solid #BFDBFE', borderRadius: '16px', padding: '16px', marginBottom: '24px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '50%', backgroundColor: '#DBEAFE', flexShrink: 0, overflow: 'hidden' }}>
                  <img src={record.partner_image || '/patient.png'} alt="patient" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '17px', fontWeight: 'bold', color: '#111827' }}>{record.partner_name} 환자</div>
                </div>
                <span style={{ border: `1.5px solid ${DOCTOR_GREEN}`, color: DOCTOR_GREEN, borderRadius: '20px', padding: '4px 12px', fontSize: '13px', fontWeight: '600' }}>완료</span>
              </div>
              <div style={{ borderTop: '1px solid #E5E7EB', paddingTop: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button
                  onClick={() => navigate(`/doctor/prescription-view/${record.id}`)}
                  style={{ backgroundColor: '#F0FDF4', color: DOCTOR_GREEN, border: 'none', borderRadius: '20px', padding: '6px 14px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', fontFamily: 'Arial, sans-serif' }}
                >
                  처방전 확인
                </button>
              </div>
                ...
                <div key={msg.id} style={{ backgroundColor: msg.sender === 'doctor' ? '#F0FDF4' : '#EFF6FF', borderRadius: '12px', padding: '12px 14px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                    <div style={{ width: '28px', height: '28px', borderRadius: '50%', backgroundColor: msg.sender === 'doctor' ? DOCTOR_GREEN : PATIENT_BLUE, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <span style={{ color: '#fff', fontSize: '11px', fontWeight: 'bold' }}>{msg.sender === 'doctor' ? '나' : '환자'}</span>
                    </div>
                    <span style={{ fontSize: '13px', fontWeight: '600', color: msg.sender === 'doctor' ? DOCTOR_GREEN : PATIENT_BLUE }}>
                      {msg.sender === 'doctor' ? '나 (음성)' : `${record.partner_name} 환자 (수어 번역)`}
                    </span>
                  </div>
